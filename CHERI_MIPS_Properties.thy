...
(*lemma get_regval_simps:
  "get_regval ''InstCount'' s = Some (Regval_int (InstCount s))"
  "get_regval ''CID'' s = Some (regval_of_bitvector_64_dec (CID s))"
  by (auto simp: register_defs regval_of_int_def)*)
...
(*context CHERI_MIPS_ISA
begin

lemma
  assumes "reads_regs_from R t s"
    and "\<forall>r \<in> R. get_regval r s' = get_regval r s"
  shows "reads_regs_from R t s'"
  using assms
  apply (induction R t s rule: reads_regs_from.induct)
  apply (auto split: if_splits option.splits)
  oops

lemma
  assumes "reads_regs_from R t regs"
    and "R' \<subseteq> R"
  shows "reads_regs_from R' t regs"
  using assms
  apply (induction R t regs rule: reads_regs_from.induct)
  apply (auto split: if_splits option.splits)
  oops
...
lemma
  ...
  apply (traces_enabledI assms: assms)
  oops
...
(*lemma Run_instr_fetch_iff_fetch[simp]:
  "Run (instr_fetch ISA) t instr \<longleftrightarrow> Run (fetch u) t (Some instr)"
  by (auto simp: ISA_def elim!: Run_bindE split: option.splits
           intro: Traces_bindI[of _ _ _ _ "[]", simplified])*)

definition "state_assms t s \<equiv> reads_regs_from trans_regs t trans_regstate \<and> reads_regs_from {''PCC''} t s"
definition "fetch_assms t \<equiv> (\<exists>s. state_assms t s)"
definition "instr_assms t \<equiv> (\<exists>s. state_assms t s \<and> CHERI_MIPS_Reg_Automaton.invariant s)"
...
proof
  ...
  then obtain s
    where reg_assms: "reads_regs_from {''PCC''} t s" "Reg_Axioms.invariant s"
      and mem_assms: "reads_regs_from trans_regs t trans_regstate"
  ...
  show "cheri_axioms CC ISA False (instr_raises_ex ISA instr t)
    ...
    using Reg_Axioms.hasTrace_reg_axioms[OF t reg_assms]
    using Mem_Axioms.hasTrace_mem_axioms[OF t mem_assms]
    by auto
next
  fix t
  interpret Reg_Axioms: CHERI_MIPS_Reg_Automaton
    where ex_traces = "fetch_raises_ex ISA t"
      ...
      and translate_address = translate_address .
  interpret Mem_Axioms: CHERI_MIPS_Mem_Automaton trans_regstate by unfold_locales
  assume "hasTrace t (instr_fetch ISA)" and "fetch_assms t"
...
next
  ...
  then have "updates_regs trans_regs t trans_regstate = Some trans_regstate"
    using Mem_Axioms.preserves_invariant_execute[of instr]
    by (elim runs_preserve_invariantE[where t = t and s = trans_regstate and a = "()"])
       (auto simp: instr_assms_def state_assms_def trans_inv_def regstate_simp)
...
next
  ...
  moreover have "updates_regs trans_regs t trans_regstate = Some trans_regstate"
    using * **
    by (elim runs_preserve_invariantE[OF Mem_Axioms.preserves_invariant_instr_fetch])
       (auto simp: trans_inv_def fetch_assms_def state_assms_def regstate_simp)
  ultimately show "fetch_assms t \<and> instr_assms t'"
    unfolding instr_assms_def fetch_assms_def state_assms_def
    by (auto simp: regstate_simp ISA_def elim!: Run_bindE split: option.splits
             dest!: Reg_Axioms.fetch_establishes_inv)
qed

lemma translate_address_tag_aligned:
  fixes s :: "regstate sequential_state"
  assumes "translate_address vaddr load s = Some paddr"
