...
record Capability  =
 ...
 Capability_tag ::" bool " 
     ...
     Capability_padding ::" 8 bits " 

     Capability_otype ::" 24 bits " 

     Capability_uperms ::" 16 bits " 

     Capability_perm_reserved12_14 ::" 3 bits " 
     ...
     Capability_base ::" 64 bits " 

     Capability_length ::" 64 bits "  
...
fun int_of_regval  :: \<open> register_value \<Rightarrow>(int)option \<close>  where 
     \<open> int_of_regval (Regval_int (v)) = ( Some v )\<close> 
  for  v  :: " int "
|\<open> int_of_regval _ = ( None )\<close>


\<comment> \<open>\<open>val regval_of_int : ii -> register_value\<close>\<close>

definition regval_of_int  :: \<open> int \<Rightarrow> register_value \<close>  where 
     \<open> regval_of_int v = ( Regval_int v )\<close> 
  for  v  :: " int "
...
definition InstCount_ref  :: \<open>((regstate),(register_value),(int))register_ref \<close>  where 
     \<open> InstCount_ref = ( (|
  name = (''InstCount''),
  read_from = ((\<lambda> s . (InstCount   s))),
  write_to = ((\<lambda> v s .  (( s (| InstCount := v |))))),
  of_regval = ((\<lambda> v .  int_of_regval v)),
  regval_of = ((\<lambda> v .  regval_of_int v)) |) )\<close>
